...
- Исправлен критический баг: все сделки ссылались на stage_id из чужих аккаунтов
- Симптом: канбан показывал пустые карточки, модалки грузили только скелет
- Решение: автоматическое исправление stage_id для всех аккаунтов
- API фиксы: `/api/deals/[id]/full` (ru.full_name вместо ru.first_name), async params для Next.js 15
- Коммит: `56f1f2a`

**New Deal Modal UX Fix (Dec 7, 2025):**
- Модалка новой сделки больше не закрывается автоматически после сохранения
- Симптом: при создании сделки модалка закрывалась, при редактировании - оставалась открытой (несогласованность UX)
- Решение: убран `onClose(true)` из блока создания, добавлено обновление канбан-доски через event
- Результат: единообразное поведение - модалка всегда остаётся открытой после сохранения
- Коммит: `4ece16c`

**Deal Save Protection Fix (Dec 7, 2025):**
- Защита от повторного сохранения сделки - добавлен флаг `isSaving`
- Симптом: можно было многократно нажимать "Сохранить", создавая дубликаты
- Решение: блокировка кнопки во время сохранения, визуальная обратная связь ("Сохранение..." → "✓ Сохранено")
- Коммит: `5b3f28a`

**Kanban Cards Display Fix (Dec 7, 2025):**
- Добавлено отображение компании и контактов на канбан-карточках сделок
- Симптом: карточки показывали только название и бюджет, без компании и контактов
- Решение: обновлён API `/api/pipelines/[id]/deals` с CTE для контактов, компонент `DraggableCard` отображает компанию (🏢) и контакты (👤)
- Результат: канбан-карточки показывают полный контекст сделки
- Коммит: `1a60334`

**Deal Modal Loading Fix (Dec 7, 2025):**
- Исправлена задержка отображения данных при открытии модалки сделки
- Симптом: желтое отладочное окно появлялось на 1-3 секунды перед показом данных
- Причина: `isReady` устанавливался в `true` ДО загрузки данных из API
- Решение: перемещён `setIsReady(true)` в блок `.then()` после получения данных
- Результат: плавный skeleton с анимацией pulse во время загрузки
- Коммит: `a20ab2b`

**UI/UX Improvements: Currency Removal & Enhanced Deal Cards (Dec 9, 2025):**
- Убрана валюта из всего проекта - теперь везде жёстко `₽`
- Убраны стрелки вверх/вниз у поля бюджета (чистый input)
- Уменьшены вертикальные отступы в модалке сделки для компактности
- "Отв-ный" → "Ответственный" (полное написание)
- **Улучшена карточка канбана:**
  - Добавлен ответственный под названием (🔑, светлее, жирнее, крупнее)
  - Контакты теперь по одному на строку (👥)
  - Бюджет и дата перенесены вниз с разделителем
  - Бюджет синий, жирный; дата серая с полным форматом (ДД.ММ.ГГГГ, ЧЧ:ММ)
- Добавлена кнопка "Сохранить и выйти" в диалог несохранённых изменений
- **Изменённые файлы:** DealModal.tsx, KanbanBoard.tsx, все API routes с валютой
- Коммит: `d23c9c9`

**Deal Modal Footer & Icons Redesign (Dec 9, 2025):**
- **Иконки добавления:** SVG круг с плюсом (outline) вместо символов ⊞/⊕
- **Все числовые поля:** убраны стрелки спиннеров (Бюджет, Бюджет 2)
- **Новый подвал модалки:**
  - Появляется только при изменениях (`hasChanges`)
  - Анимации: slideUp (появление 300ms), slideDown (уход 300ms)
  - Кнопки: "Сохранить" (зелёная), "Отмена" (серая), "Сохранить и выйти" (синяя справа)
  - Position: absolute bottom-0 поверх контента с тенью
- **Кнопка удаления:** перенесена в хедер (слева от крестика), SVG иконка корзины
- **Система восстановления:** сохранение initialEditForm, initialDealContacts, initialCompanies, а также драфтов новых контактов и компаний
- **Автоопределение "нет изменений":** рекурсивное сравнение `deepEqualWithEmpty` учитывает пустые значения и драфты, ручной возврат значений убирает футер без доп. действий
- **Новая форма контакта:** поле "Добавить контакт" работает как автокомплит и драфт, никаких отдельных кнопок создания; данные драфта сохраняются в памяти до общего сохранения сделки

**UI/UX Refinements (Dec 10, 2025):**
- **Поле "Компания" в контактах:** сделано редактируемым с автоподстановкой из сделки и автодополнением
- **Упрощение полей компании:** убрано контекстное меню, поле стало обычным input'ом как все остальные поля контакта
- **Компактность карточек:**
  - Высота свернутых контактов: 60px → 40px
  - Padding контактов: 8-10px → 4px
  - Padding компаний: 8px → 4px, высота 60px → 40px
  - Вертикальные отступы заголовков: py-0.5/py-2 → py-0
- **Оптимизация канбана:**
  - Padding колонок этапов: 1rem (16px) → 0.5rem (8px)
  - Ширина колонок: w-72 (288px) → фиксированные 310px
- **Бюджет:** при загрузке показывается целое число без дробной части (0 вместо 0.00)
...
- 2 аккаунта: Admin User + Elestet
- 6 сделок (4 + 2), все корректно привязаны к этапам своих воронок
- Все сделки отображаются в канбане с названиями, бюджетами, компаниями
